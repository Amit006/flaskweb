...
def Dashboard():
    ...
    if request.method == 'GET':
        ...
        return render_template('Dashboard.html', timeline=_get_timeline(user_id))
        text = request.form['tweet']
        post_id = str(db.incr('next_post_id'))


    postKeyName = 'post:' + post_id
    db.hmset(postKeyName, dict(user_id=user_id,
                               ts=str(datetime.utcnow()), text=text))

    postsKeyName = 'posts:' + str(user_id)
    db.lpush(postsKeyName, str(post_id))

    timeLineKeyName = 'timeline:' + str(user_id)
    db.lpush(timeLineKeyName, str(post_id))
    db.ltrim(timeLineKeyName, 0, 100)

    return render_template('Dashboard.html', timeline=_get_timeline(user_id))


def _get_timeline(user_id):
    posts =('timeline:' + str(user_id), 0, -1)
    timeline = []
    for post_id in posts:
        post =('post:' + str(post_id, 'utf-8'))
        timeline.append(dict(
            username=('users:' + str(post[b'user_id'], 'utf-8'), 'username'),
            ts=post[b'ts'],
            text=post[b'text']))
    return timeline
...
#     cur = mysql.connection.cursor()
#     cur.execute("""INSERT INTO users VALUES (DEFAULT, %s,%s,%s)""",(UName ,Email ,UPassword))
#
